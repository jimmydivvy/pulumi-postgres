PROJECT_NAME := Postgres Package
include build/common.mk

PACK             := postgresql
PACKDIR          := sdk
PROJECT          := github.com/jimmydivvy/pulumi-postgres
NODE_MODULE_NAME := @jimmydivvy/pulumi-postgres
...
build::
	go install -ldflags "-X github.com/jimmydivvy/pulumi-postgres/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${TFGEN}
	go install -ldflags "-X github.com/jimmydivvy/pulumi-postgres/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
...
install::
	GOBIN=$(PULUMI_BIN) go install -ldflags "-X github.com/jimmydivvy/pulumi-postgres/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
